...
Viewport world lighting is explicit in `src/renderer/viewportWorld.ts`. Light mode is the existing bright background/ground/light setup, and dark mode only changes renderer-owned world colors and light intensities. The default WebGPU viewport intentionally does not attach scene fog so large assets stay readable when the camera is zoomed out; any fog values in the world environment are not part of the default viewport render. It must not mutate asset materials or app chrome, though assets will naturally render under the selected world lights.

Default viewport camera clipping is controlled by `defaultViewportCameraConfig` in `src/renderer/viewportCamera.ts`; the far plane is intentionally set beyond the OrbitControls max zoom-out distance and is the first place to hand-tune default/path-tracer shared camera clipping. Default WebGPU contact shadows are controlled separately by `src/renderer/viewportShadows.ts`. `defaultViewportShadowConfig` sets the minimum quality/coverage knobs, while `computeDefaultViewportShadowState()` dynamically fits the key directional light's shadow camera to the asset bounds plus the asset's projected ground shadow. This matters for tall assets: a fixed low directional-light position can leave upper geometry behind/outside the shadow camera even when the world plane is large enough. Keep the map size high enough to avoid visibly mushy shadows when dynamic bounds expand.
...
Path tracer mode also has renderer-mode-specific navigation behavior: OrbitControls damping is disabled and selected-asset target centering snaps immediately. The hidden WebGPU canvas still owns picking and camera state, but path tracer mode should not leave residual inertia after pan/orbit input because any lingering camera movement delays progressive sample accumulation. While direct OrbitControls input is active, `PathTracingCanvas` caps live accumulation to `pathTracingConfig.interaction.activeSampleLimit` so dragging, panning, and wheel bursts render only a coarse preview sample; after the viewport navigation settle delay, normal accumulation resumes from the current camera. Default mode keeps the existing damped camera behavior.
