...
Context budgeting uses the compiled provider request, not just the replay text persisted in agent sessions. The estimate includes the system prompt, user prompt, current candidate/selected asset context when present, and a small image reserve. The safe input budget is the model context limit minus the larger of the fixed harness buffer and the selected provider max-output token budget, so default 64k-output runs reserve 64k rather than only the older 50k buffer. OpenAI Responses and Gemini Interactions can start a fresh continuation session when the accumulated provider-side context approaches the safe input budget; OpenRouter and other stateless chat providers do not accumulate provider-side context, so each compiled request is checked independently. If a single reconstructed request cannot fit a fresh safe context, the loop fails before sending it to avoid opaque provider transport/body-shape failures.
