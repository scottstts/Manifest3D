...
Provider context uses a three-tier strategy. OpenAI is tier 1: the loop persists the latest Responses `responseId` and follow-up turns pass it as `previous_response_id`, sending only the new delta prompt and any genuinely new turn images. Gemini is tier 2: native Gemini does not use response-id continuation; the loop compiles stable context into `cachedContents`, persists `cachedContentName`, `cacheExpiresAt`, provider/model identity, cache key, and source media ids, then follow-up `generateContent` calls reference `cachedContent` and send only dynamic feedback. When the stable Gemini context changes, such as a new repair candidate JSON, the next request creates a new cachedContent handle. OpenRouter is tier 3: it remains stateless local replay with prompt-cache best effort.
...
OpenRouter calls keep top-level `session_id` for sticky routing/cache locality and must not send throughput sorting, because OpenRouter prioritizes explicit sort routing over session stickiness. OpenRouter request replay is transcript-shaped: each repair turn sends the previous user prompt and previous assistant JSON as chat history, then appends only the new repair delta. This lets provider prompt caching grow past the system prompt into the large generated asset or patch history. A generated assistant output cannot be read from cache on the immediate next request because it was not part of any previous prompt; that next request writes it into the prompt-cache prefix so later repair turns can read it.

Anthropic-family OpenRouter models use top-level automatic prompt caching with `cache_control: { "type": "ephemeral", "ttl": "1h" }`; do not add an explicit system-only cache breakpoint for Anthropic because that can cap observed cache reads at the system/schema prefix. Explicit-cache OpenRouter models such as Qwen keep content-block cache markers, but the moving marker belongs on the latest user block so the cached prefix advances with the transcript.
...
The LLM transcript is not persisted as a full conversation. Persisted session exchanges are archive/debug/audit data. Provider-owned context handles are the primary continuation mechanism when available: OpenAI uses response ids, Gemini uses cachedContent names, and OpenRouter reconstructs stateless prompt context client-side.

Context budgeting uses the compiled provider request, not just the replay text persisted in agent sessions. The estimate includes the system prompt, user prompt, current candidate/selected asset context when present, and a small image reserve. The safe input budget is the model context limit minus the larger of the fixed harness buffer and the selected provider max-output token budget, so default 64k-output runs reserve 64k rather than only the older 50k buffer. OpenAI Responses can start a fresh continuation session when accumulated provider-side context approaches the safe input budget; Gemini checks the active stable cache plus dynamic prompt for the current turn, and OpenRouter checks each reconstructed stateless request independently. If a single request cannot fit a fresh safe context, the loop fails before sending it to avoid opaque provider transport/body-shape failures.
