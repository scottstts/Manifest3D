...
describe('compileManifestPrompt', () => {
  ...
  it('includes candidate JSON and validation feedback for repair prompts', () => {
    ...
    expect(compiled.user).toContain('<candidate_json>\n{"schemaVersion":2')
    expect(compiled.user).toContain('`tool` must be `apply_manifest_patch`')
    expect(compiled.user).toContain('Patch Tool Example')
    expect(compiled.user).toContain('valueJson')
    expect(compiled.user).toContain('Never use root path "", `/asset`, `/assets`')
